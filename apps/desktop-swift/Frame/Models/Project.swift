...
/// Represents a single zoom segment in the timeline
struct ZoomSegment: Codable, Identifiable, Hashable {
    let id: UUID
    var startTime: Double      // seconds from video start
    var duration: Double       // seconds
    var scale: Double          // zoom level (1.0 = no zoom)
    var isEnabled: Bool        // disabled zooms shown at reduced opacity
    var isAutoGenerated: Bool  // from cursor clicks vs manually added
    
    init(id: UUID = UUID(), startTime: Double, duration: Double, scale: Double, isEnabled: Bool = true, isAutoGenerated: Bool = false) {
        self.id = id
    ...
    /// Check if this zoom is active at a given time
    func isActive(at time: Double) -> Bool {
        isEnabled && time >= startTime && time < endTime
    }
    
    /// Get the progress within this zoom (0.0...1.0)
